      ...
      <div className="max-w-full px-6 md:px-12 mx-auto py-[24px]">
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <a href="/" className={`font-playfair font-bold text-secondary-foreground transition-all duration-300 ${isScrolled ? 'text-lg' : 'text-xl'}`}>
              Brand Strategy
            </a>
          </div>
          ...
          <button className={`lg:hidden transition-all duration-300 ${isScrolled ? 'opacity-100' : 'opacity-100 lg:opacity-0'}`} onClick={toggleMenu} aria-label="Toggle menu">
            {isMenuOpen ? <X className="h-6 w-6 text-secondary-foreground" /> : <Menu className="h-6 w-6 text-secondary-foreground" />}
          </button>
        </div>

        {/* Mobile Navigation Menu */}
        {isMenuOpen && <div className="lg:hidden mt-4 py-4 animate-fade-in bg-white/95 backdrop-blur-sm rounded-lg">
